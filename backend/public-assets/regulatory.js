...
async function loadDepartmentFromServer(dept) {
  try {
    const res = await fetch(`${API_BASE}/api/regulations/${encodeURIComponent(dept)}?limit=${pageSize}&page=${currentPage}`);
    let data = [];
    try {
      data = await res.json();
    } catch (_) { data = []; }
    ...
    try {
      const sRes = await fetch(`${API_BASE}/api/regulations/${encodeURIComponent(dept)}/stats`);
